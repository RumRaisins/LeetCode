...
namespace haizei {
	...
	public:
		...
		ostream &operator <<(const int x) {
			char fmt[15];
			sprintf(fmt, "%%.%dd", this->__intprecision);
			this->__Print(fmt, x);
			return *this;
		}
		ostream &operator <<(const double x) {
			char fmt[15];
			sprintf(fmt, "%%.%dlf", this->__doubleprecision);
			this->__Print(fmt, x);
			return *this;
		}
		...
		ostream &operator <<(const Precision &a) {
			this->__doubleprecision = a.precision;
			return *this;
		}
		ostream &operator <<(const IntControl &a) {
			this->__intprecision = a.precision;
			return *this;
		}
	private:
		int __doubleprecision;
		int __intprecision;
		template<typename T>
...
int main() {
	...
	//Point p2 = { 4, 7 };
	//Point p3 = { 10, 96 };
	//haizei::cout << haizei::IntControl(3) << 523 << haizei::endl;
